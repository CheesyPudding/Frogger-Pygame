...
def drawText(text, text_font, surface, x_param, y):
    textobj = text_font.render(text, 1, (255, 255, 255))
    textrect = textobj.get_rect()
    textrect.topleft = (x_param, y)
    surface.blit(textobj, textrect)


def waitForPlayerToPressKey():
    while True:
        for press_event in pygame.event.get():
            if press_event.type == QUIT:
                pygame.quit()
                sys.exit()
            if press_event.type == KEYDOWN:
                if press_event.key == K_ESCAPE:  # pressing escape quits
                    pygame.quit()
                    sys.exit()
                return
...
for x in range(0):
    blocks.append(platform(WINDOWWIDTH, random.randint(4 * BLOCKSIZE, WINDOWHEIGHT - (3 * BLOCKSIZE)), BLOCKSIZE * 5,
                           BLOCKSIZE / 2))
...
drawText('Frogger', font, windowSurface, (WINDOWWIDTH / 3), (WINDOWHEIGHT / 4))
drawText('Press a key to start', font, windowSurface, (WINDOWWIDTH / 3) - 30, (WINDOWHEIGHT / 4) + 50)
drawText('Controls: hold arrow keys or a/s/d/f to move up/down/left/or right', font2, windowSurface, (WINDOWWIDTH / 7),
         (WINDOWHEIGHT / 2))
drawText('Objective: endless 2D platformer, try to survive as long as possible', font2, windowSurface,
         (WINDOWWIDTH / 8), (WINDOWHEIGHT / 2) + 50)
drawText('by jumping on the platforms that vary in speed and change direction', font2, windowSurface, (WINDOWWIDTH / 8),
         (WINDOWHEIGHT / 2) + 80)
...
while True:
    ...
    if blockCounter >= NEWBLOCK:
        ...
        if gravDirection == 1:
            blocks.append(
                pygame.Rect(random.randint(BLOCKSIZE, WINDOWWIDTH - BLOCKSIZE), 0, BLOCKSIZE * 4, BLOCKSIZE / 2))
        if gravDirection == 2:
            blocks.append(
                pygame.Rect(WINDOWWIDTH, random.randint(4 * BLOCKSIZE, WINDOWHEIGHT - (3 * BLOCKSIZE)), BLOCKSIZE * 4,
                            BLOCKSIZE / 2))
        if gravDirection == 3:
            blocks.append(
                pygame.Rect(0 - (3 * BLOCKSIZE), random.randint(4 * BLOCKSIZE, WINDOWHEIGHT - (3 * BLOCKSIZE)),
                            BLOCKSIZE * 4, BLOCKSIZE / 2))
